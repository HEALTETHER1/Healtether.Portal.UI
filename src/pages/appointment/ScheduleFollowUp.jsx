...
function ScheduleFollowUp({timeSlots,onSave,selectedDate, setSelectedDate}) {
    const currentDate = new Date()
        .toISOString()
        .split('T')[0];
    const [selectedFollowUpTimeSlot,SetFollowUpTimeSlot]=useState(0);
    const [notify,setNotify]=useState(false);
    return (
        <div className="flex flex-col">
            <ContainerHeading heading={"Schedule Follow-Up"}/>
            <div className="flex flex-row mt-px">
                ...
                <div className="w-1/2 pr-2">
                    <label className="text-sm ">Follow-Up Time Slot</label>
                    <select
                        className={DefaultTextboxClass + " w-full leading-8"}
                        name="followUpTimeSlot"
                        value={selectedFollowUpTimeSlot}
        ...
        <div className="mt-3 ">
            <button type="button" className="bg-BgDisabled  border border-Primary inline-block rounded-md text-md font-normal leading-normal text-Primary  px-20 py-2" onClick={()=>onSave(selectedDate,selectedFollowUpTimeSlot)}>Done</button>
        </div>
        </div>
    );
